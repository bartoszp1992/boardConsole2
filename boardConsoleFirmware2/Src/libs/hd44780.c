...
void LCD_WriteCmd(uint8_t cmd) {
	...
	LCD_WriteByte(cmd);
#ifndef USE_RW
	delay_us(2000); //<<--- wait for command processing
#endif
}
...
//
//	Write string with known length
//
void LCD_Data(char *data, uint8_t size) {
...
void LCD_Blink(uint8_t on_off) {
	if (on_off == 0)
		...
		LCD_WriteCmd(
				LCDC_ONOFF | LCDC_DISPLAYON | LCDC_CURSORON | LCDC_BLINKON);
}
